...
export default function Cashier() {
  ...
  const handleCheckout = async () => {
    ...
    // Find today's date prefix (YYMMDD format) as generated by generateInvoiceNumber
    const now = new Date();
    const yy = String(now.getFullYear()).slice(-2);
    const mm = String(now.getMonth() + 1).padStart(2, "0");
    const dd = String(now.getDate()).padStart(2, "0");
    const datePart = `${dd}${mm}${yy}`; // e.g., "260711"

    // 1. Find all sequence numbers from active transactions today
    ...
    // 2. Find all sequence numbers referenced in activity logs today
    ...
    // Start generating from the maximum sequence ever used today
    let invoiceCount = maxSequence;
    let invoiceNumber = generateInvoiceNumber(invoiceCount);
