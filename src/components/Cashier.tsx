...
export default function Cashier() {
  ...
  const handleCheckout = async () => {
    ...
    // Find selected date's prefix (YYMMDD format) as generated by generateInvoiceNumber
    const now = new Date();
    ...
    const datePart = `${yy}${mm}${dd}`; // YYMMDD format

    // 1. Find all sequence numbers from active transactions on the selected date
    const activeSequences = txs
      .filter((tx) => tx.invoiceNumber && tx.invoiceNumber.startsWith(`INV-${datePart}-`))
      .map((tx) => {
    ...
    // 2. Find all sequence numbers referenced in activity logs on the selected date
    const logRegex = new RegExp(`INV-${datePart}-(\\d{3,5})`, "g");
    const logSequences: number[] = [];
    logs.forEach((log) => {
      if (log.description) {
        logRegex.lastIndex = 0;
        let match;
        while ((match = logRegex.exec(log.description)) !== null) {
          const num = parseInt(match[1], 10);
          if (!isNaN(num)) {
            logSequences.push(num);
          }
        }
      }
    });

    const allSequences = [...activeSequences, ...logSequences];
    const maxSequence = allSequences.length > 0 ? Math.max(...allSequences) : 0;

    // Start generating from the maximum sequence ever used on the selected date
    let invoiceCount = maxSequence;
    let invoiceNumber = generateInvoiceNumber(invoiceCount, dateObj);

    // Robust collision prevention check: auto-increment sequence if there is a match in db
    while (txs.some((tx) => tx.invoiceNumber === invoiceNumber)) {
      invoiceCount++;
      invoiceNumber = generateInvoiceNumber(invoiceCount, dateObj);
    }
    ...
    try {
      ...
      alert(`GAGAL MENYIMPAN TRANSAKSI:\n${error.message || error}\n\nData transaksi belum terhapus dari keranjang, silakan coba simpan kembali.`);
    }
  };
        ...
        <div className="bg-white rounded-2xl border border-slate-200/50 p-5 sm:p-6 shadow-sm space-y-5 relative">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                id="cashier-transaction-date-input"
                type="date"
                value={transactionDate}
                onChange={(e) => setTransactionDate(e.target.value)}
                className="w-full rounded-xl border border-slate-200 bg-slate-50/50 py-2.5 pl-9 pr-4 text-xs font-bold text-slate-900 focus:border-red-500 focus:ring-1 focus:ring-red-500 focus:outline-none transition-all duration-200 cursor-pointer"
              />
            </div>
          </div>
